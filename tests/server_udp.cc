...
/** "Generic" receive_vector with the connector library. T has to be default initializable and streamable because of debugging
  ...
  */
template < class T >
void receive_vector(connector::server< connector::UDP > & server, std::vector< T > & vec)
{   
    // remove all elements from the vector
    vec.clear();
    // preallocate the size "member"
    uint32_t current_vec_size = 0;
    server.receive_udp< uint32_t >( current_vec_size );
    ...
    {
        server.receive_udp< T >( element_memory );
...
int main()
{
    ...
    const int port = 4444;
    // call the server with it's constructor and the template parameter where we specify if we use TCP or UDP
    ...
    // PODS
    server.receive_udp< char >( c );
    DEBUG_MSG_CONNECTOR("Got char: " << c << '\n');
    server.receive_udp< uint32_t >( i );
    DEBUG_MSG_CONNECTOR("Got uint32_t: " << i << '\n');
    server.receive_udp< double >( d );
    DEBUG_MSG_CONNECTOR("Got double: " << d << '\n');

    // struct
    server.receive_udp< struct structure::complex_struct >( cs );
    ...
    server.receive_udp< uint32_t >( string_length );
    DEBUG_MSG_CONNECTOR("Got string_length: " << string_length << '\n');
    server.receive_udp< char >( message[0], sizeof(char) * string_length );
