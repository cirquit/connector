...
/** "Generic" send_vector with the connector library. T has to be default initializable and streamable because of debugging
  ...
  */
template < class T >
void send_vector( connector::client< connector::UDP > & client, std::vector< T > & vec )
{
...
int main()
{
    ...
    // call the client with it's constructor and the template parameter where we specify if we use TCP or UDP
    ...
    client.send_udp< struct structure::complex_struct >( cs );
    std::this_thread::sleep_for(500ms);
    ...
    client.send_udp< uint32_t >( string_length );
    DEBUG_MSG_CONNECTOR("Sending message: " << message << '\n');
    client.send_udp< char >( message[0], sizeof(char) * string_length );
    std::this_thread::sleep_for(500ms);
